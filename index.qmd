...
print(f"Probability of final balance > ${initial_balance:,}: {prob_above_initial:.3f}")
...
print(f"Probability estimate P(final > ${initial_balance:,}): {prob_above_30000:.3f}")
...
# Implement the modified game where only 25% of balance is at risk each round
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.ticker import FuncFormatter, MultipleLocator
...
n_flips = 29
initial_balance = 30000
gamble_fraction = 0.25  # Only 25% of balance is gambled each round
...
# Create comparison plot with both distributions
...
# Plot 1: Original Strategy
balances_below_orig = final_balances[final_balances < initial_balance]
balances_above_orig = final_balances[final_balances >= initial_balance]
ax1.hist(balances_below_orig, bins=log_bins, edgecolor='black', alpha=0.7, color='red', 
        label=f'Below ${initial_balance:,}')
ax1.hist(balances_above_orig, bins=log_bins, edgecolor='black', alpha=0.7, color='green', 
        label=f'Above ${initial_balance:,}')
ax1.axvline(x=initial_balance, color='red', linestyle='--', linewidth=2, 
           label=f'Initial (${initial_balance:,})')
...
ax1.set_ylabel('No of Simulations', fontsize=12)
ax1.set_title('Original Strategy: Full Balance at Risk', fontsize=14, fontweight='bold')
...
# Plot 2: Modified Strategy
balances_below_mod = final_balances_modified[final_balances_modified < initial_balance]
balances_above_mod = final_balances_modified[final_balances_modified >= initial_balance]
ax2.hist(balances_below_mod, bins=log_bins, edgecolor='black', alpha=0.7, color='red', 
        label=f'Below ${initial_balance:,}')
ax2.hist(balances_above_mod, bins=log_bins, edgecolor='black', alpha=0.7, color='green', 
        label=f'Above ${initial_balance:,}')
ax2.axvline(x=initial_balance, color='red', linestyle='--', linewidth=2, 
           label=f'Initial (${initial_balance:,})')
ax2.axvline(x=mean_modified, color='green', linestyle='-', linewidth=2, 
           label=f'Mean (${mean_modified:,.0f})')
ax2.axvline(x=median_modified, color='orange', linestyle=':', linewidth=2, 
           label=f'Median (${median_modified:,.0f})')
ax2.set_xscale('log')
ax2.set_xlabel('Final Balance ($, log scale)', fontsize=12)
ax2.set_ylabel('No of Simulations', fontsize=12)
ax2.set_title('Modified Strategy: 25% of Balance at Risk', fontsize=14, fontweight='bold')
...
print(f"  Mean final balance: ${mean_final:,.2f}")
print(f"  Median final balance: ${median_final:,.2f}")
print(f"  P(final > ${initial_balance:,}): {prob_above_initial:.3f}")
...
print(f"  P(final > ${initial_balance:,}): {prob_above_initial_modified:.3f}")
...
print(f"  P(>${initial_balance:,}) difference: {prob_above_initial_modified - prob_above_initial:.3f}")
